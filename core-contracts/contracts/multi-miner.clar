...
(define-read-only (make-registration-hash (ft-data {ft-contract: <ft-trait>, deposit-fn-name: (string-ascii 45)}))
    (let ((data-buff (unwrap-panic (to-consensus-buff (merge registration-data { multi-contract: CONTRACT_ADDRESS }))))
          (data-hash (sha256 data-buff))
          ;; in 2.0, this is a constant: 0xe2f4d0b1eca5f1b4eb853cd7f1c843540cfb21de8bfdaa59c504a6775cd2cfe9
          (structured-hash (sha256 (concat sip18-data-prefix data-hash))))
          structured-hash))
...
# func that needs to be called
(define-public (register-new-ft-contract (ft-data {ft-contract: <ft-trait>, deposit-fn-name: (string-ascii 45)})
                              (signatures (list 9 (buff 65))))
    (let ((registration-hash (make-registration-hash ft-data))
         ...
         (as-contract (contract-call? .hyperchains register-new-ft-contract (get ft-contract ft-data) (get deposit-fn-name ft-data)))))

(define-public (register-new-nft-contract (nft-data {nft-contract: <nft-trait>, deposit-fn-name: (string-ascii 45)})
                              (signatures (list 9 (buff 65))))
    (let ((registration-hash (make-registration-hash nft-data))
          (signer-principals (try! (fold verify-sign-helper signatures (ok { registration-hash: registration-hash, signers: (list) })))))
         ;; check that the caller is a direct caller!
         (asserts! (is-eq tx-sender contract-caller) (err ERR_UNAUTHORIZED_CONTRACT_CALLER))
         ;; check that we have enough signatures
         (try! (check-miners (append (get signers signer-principals) tx-sender)))
         ;; execute the block commit
         (as-contract (contract-call? .hyperchains register-new-nft-contract (get nft-contract nft-data) (get deposit-fn-name nft-data)))))
