...
fn ft_deposit_failure_and_refund_integration_test() {
    ...
    // Mint 10 ft-tokens for user on L1 chain
    let l1_mint_ft_tx = make_contract_call(
        &MOCKNET_PRIVATE_KEY_1,
        LAYER_1_CHAIN_ID_TESTNET,
        l1_nonce,
        ...
        "simple-ft",
        "gift-tokens",
        &[Value::UInt(10), Value::Principal(user_addr.into())],
    );
    l1_nonce += 1;
    ...
    // Check that user has 0 tokens on L2
    let res = call_read_only(
        &l2_rpc_origin,
        &user_addr,
        "simple-ft",
    ...
    // Deposit 3 ft-tokens into subnet contract on L1
    submit_tx(&l1_rpc_origin, &l1_deposit_ft_tx);
    ...
    // Check that the deposit failed and user still has 0 tokens on L2
    let res = call_read_only(
        &l2_rpc_origin,
        &user_addr,
        "simple-ft",
    ...
    // Check that the user now only owns 7 ft on the L1. Contract should own the rest
    let res = call_read_only(
        &l1_rpc_origin,
        &user_addr,
        "simple-ft",
    ...
    assert_eq!(amount, Value::okay(Value::UInt(7)).unwrap());

    // Check that the subnet contract owns 3 ft on the L1
    let subnet_contract_principal = Value::Principal(PrincipalData::Contract(
        config.burnchain.contract_identifier.clone(),
    ));
    let res = call_read_only(
    ...
    assert_eq!(amount, Value::okay(Value::UInt(3)).unwrap());

    // A withdrawal event should have been automatically created upon failure, find it
    let block_data = test_observer::get_blocks();
    let mut withdraw_events = filter_map_events(&block_data, |height, event| {
        let ev_type = event.get("type").unwrap().as_str().unwrap();
        if ev_type == "contract_event" {
